...
const Cart = ({ cart, setCart }) => {
  ...
  const calculateTotal = () => {
    const totalPrice = (userId?cartData: cart).reduce((total, product) => total + Number(product.price), 0);
    return totalPrice;
  };
      ...
      { 
          ...
          loading ? (
            ...
            <div className="container-cart" style={{ width: "55%" }}>
              {(userId && cartData.length === 0) || (!userId && cart.length === 0) ? (
                <>
                ...
                <>
                  {(userId? cartData: cart).map((product) => (
